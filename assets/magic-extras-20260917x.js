...
(() => {
  ...
  function matchesIntentKind(props, intent) {
    ...
    if (kind === "restaurant") {
      return key === "amenity" && /restaurant|fast_food|food_court/.test(val);
    }
    if (kind === "cafe") {
      return key === "amenity" && /cafe|bar|pub|biergarten|nightclub/.test(val);
    }
    if (kind === "museum") {
      return (key === "tourism" && /museum|gallery/.test(val)) || val === "museum";
    }
    if (kind === "nature") {
  ...
  async function searchPlacesForIntent(destination, intent, lat, lng, title = "") {
    ...
    const pushFeature = (feature) => {
      ...
      if (!matchesIntentKind(props, intent)) return;
    ...
    if (places.length < 4) {
      const queries = [`${intent.query} ${city}`, `best ${intent.query} ${city}`, city];
    ...
    if (places.length < 3 && Number.isFinite(lat)) {
      try {
        ...
        for (const el of data.elements || []) {
          ...
          if (!name || seen.has(String(name).toLowerCase())) continue;
          if (isCountryLikePlace({ type: "node", osm_value: el.tags?.amenity || el.tags?.tourism }, name, destination)) {
            continue;
          }
          seen.add(String(name).toLowerCase());
          places.push({
          ...
          try {
            ...
            const good = hits.find((h) => {
              const t = String(h.title || "")
                ...
                .normalize("NFD")
                .replace(/\p{M}/gu, "");
              // Skip country / disambiguation pages
            ...
            if (good?.title) title = good.title;
              ...
              hits[0]?.title &&
              !/^(island|islanda|iceland)\b/i.test(hits[0].title)
            ) {
              title = hits[0].title;
            } else {
              continue; // no useful wiki hit
            }
          } catch {
            /* keep original title */
          }
  ...
  async function handleChatAsk(panel, text) {
    ...
    try {
      ...
      // Prefer a concrete city for geocoding when destination is a country alias
      ...
      let places = await searchPlacesForIntent(
        trip.destination,
        intent,
        place.lat,
        place.lng,
      ...
      if (bots.length) {
        bots[bots.length - 1].textContent = places.length
          ? `Ho trovato ${places.length} ${intentResultLabel(intent, places.length)}. Puoi aprirli su TripAdvisor o aggiungerli all’itinerario.`
