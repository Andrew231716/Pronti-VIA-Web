...
(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .pv-useful{margin:14px 0;padding:14px;border:1px solid #d9e7e7;border-radius:16px;background:linear-gradient(180deg,#f7fbfb,#fff)}
      .pv-useful h3{margin:0 0 6px;font-size:1rem;color:#16353b}
      .pv-useful .pv-sub{margin:0 0 12px;color:#5b7076;font-size:.88rem}
      .pv-useful .pv-weather{display:grid;gap:8px;margin:0 0 12px}
      .pv-useful .pv-weather-day{display:grid;grid-template-columns:88px 1fr auto;gap:8px;align-items:center;font-size:.86rem;color:#355055;padding:6px 0;border-top:1px solid #eef3f3}
      .pv-useful .pv-weather-day:first-child{border-top:0}
      .pv-useful .pv-fx{display:flex;flex-wrap:wrap;gap:8px;margin:0 0 12px}
      .pv-useful .pv-fx span{background:#eef6f6;border-radius:999px;padding:6px 10px;font-size:.8rem;font-weight:600;color:#355055}
      .pv-useful .pv-links{display:flex;flex-wrap:wrap;gap:8px}
      .pv-useful .pv-links a,.pv-useful .pv-tools button{appearance:none;border:1px solid #c9dbdb;background:#fff;color:#184047;border-radius:999px;padding:7px 12px;font:600 12px inherit;cursor:pointer;text-decoration:none}
      .pv-useful .pv-tools{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}
      .pv-useful .pv-status{min-height:1.1em;margin:8px 0 0;font-size:.82rem;color:#5b7076}
    `;
  ...
  function injectUsefulKit() {
    ...
    box.innerHTML = `
      <h3>Info utili (gratis)</h3>
      <p class="pv-sub">Meteo, cambio valute e link pratici per la destinazione — senza abbonamenti.</p>
      <div class="pv-weather" id="pv-weather">Carico il meteo…</div>
      <div class="pv-fx" id="pv-fx"></div>
      <div class="pv-links" id="pv-links"></div>
      <div class="pv-tools">
        <button type="button" id="pv-copy-day">Copia riepilogo giorno</button>
        <button type="button" id="pv-copy-trip">Copia riepilogo viaggio</button>
      </div>
  ...
  async function fillUsefulKit(box) {
    ...
    const linksEl = box.querySelector("#pv-links");
    try {
      const { data } = await loadTrip();
      const trip = data.trip;
      const place = await geocodeDestination(trip.destination);
      ...
      linksEl.innerHTML = `
        <a href="https://www.google.com/maps/search/?api=1&query=${destQ}" target="_blank" rel="noopener noreferrer">Google Maps</a>
        <a href="https://www.openstreetmap.org/search?query=${destQ}" target="_blank" rel="noopener noreferrer">OpenStreetMap</a>
        <a href="https://it.wikipedia.org/wiki/Special:Search?search=${destQ}" target="_blank" rel="noopener noreferrer">Wikipedia</a>
        <a href="https://it.wikivoyage.org/wiki/Special:Search?search=${destQ}" target="_blank" rel="noopener noreferrer">Wikivoyage</a>
        <a href="https://www.google.com/search?q=${encodeURIComponent(trip.destination + " emergenza numero")}" target="_blank" rel="noopener noreferrer">Numeri utili</a>
      `;

      // FX — Frankfurter (ECB), free, no key
      try {
        const fx = await fetch("https://api.frankfurter.dev/v1/latest?base=EUR&symbols=USD,GBP,CHF,JPY", {
          signal: AbortSignal.timeout(10000),
        }).then((r) => r.json());
        const rates = fx.rates || {};
        fxEl.innerHTML = Object.entries(rates)
          .map(([code, rate]) => `<span>1 € = ${Number(rate).toLocaleString("it-IT", { maximumFractionDigits: 2 })} ${code}</span>`)
          .join("");
        if (!fxEl.innerHTML) fxEl.textContent = "";
      } catch {
        fxEl.textContent = "";
      }
      ...
      if (!daily?.time?.length) {
        weatherEl.textContent = "Meteo non disponibile per queste date.";
        return;
      }

      weatherEl.innerHTML = "";
      const note = document.createElement("p");
      note.className = "pv-sub";
      note.style.margin = "0 0 4px";
      note.textContent =
        mode === "forecast"
          ? `Previsioni a ${trip.destination}`
          : `Media climatica (date lontane) a ${trip.destination}`;
      weatherEl.appendChild(note);

      daily.time.forEach((day, i) => {
        const row = document.createElement("div");
        row.className = "pv-weather-day";
        if (mode === "forecast") {
          ...
          row.innerHTML = `
            ...
            <span>${escapeHtml(weatherLabel(code))}${rain != null ? ` · pioggia ${rain}%` : ""}</span>
            <span>${tmin != null && tmax != null ? `${Math.round(tmin)}° / ${Math.round(tmax)}°` : "—"}</span>
          `;
        } else {
          ...
          row.innerHTML = `
            <strong>${escapeHtml(formatDayIt(day))}</strong>
            <span>${precip != null ? `Pioggia tipica ${Number(precip).toFixed(1)} mm` : "Clima medio"}</span>
            <span>${tmean != null ? `~${Math.round(tmean)}°` : "—"}</span>
          `;
        }
        weatherEl.appendChild(row);
      });
    } catch (err) {
      weatherEl.textContent = err instanceof Error ? err.message : "Info non disponibili.";
    }
  }
