...
(() => {
  ...
  async function fetchPois(lat, lng) {
    const query = `
      ...
      (
        ...
        node["tourism"="gallery"](around:10000,${lat},${lng});
        ...
        node["amenity"="place_of_worship"]["tourism"](around:8000,${lat},${lng});
      );
      out body 40;
    `;
    const endpoints = [
    ...
    throw lastError || new Error("Ricerca luoghi non disponibile.");
  }
  ...
  function renderFab() {
    ...
    if (!document.querySelector(".app-shell") && !document.querySelector("main.app-shell")) {
      // still show after trip shell appears
    }
  ...
  async function runMagicSuggest(panel) {
    ...
    try {
      ...
      panel.innerHTML = `
        ...
        <p>Proposte gratuite da OpenStreetMap intorno a <strong>${escapeHtml(trip.destination)}</strong>. Scegli cosa aggiungere all’itinerario.</p>
  ...
  async function addSelectedPois(panel) {
    ...
    try {
      ...
      status.textContent = "Fatto. Aggiorno la pagina…";
