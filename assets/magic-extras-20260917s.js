 ...
 * - Meteo sul giorno selezionato (Open-Meteo), convertitore EUR↔valuta locale in Spese,
  ...
  function injectBudgetAndCharts() {
    ...
    if (money && !money.parentElement.querySelector(".pv-budget-box")) {
      ...
      box.innerHTML = `
        ...
        <div class="row">
          <input type="text" inputmode="decimal" placeholder="Es. 1500" aria-label="Budget privato in euro" />
      ...
      button.onclick = async () => {
        try {
          ...
          db[`${user}::${auth.id}`] = cents;
          localStorage.setItem("viavia-private-budget-v1", JSON.stringify(db));
          // Best-effort: clear shared budget on the trip so collaborators don't see it.
          try {
            const { data } = await loadTrip();
            const trip = structuredClone(data.trip);
            trip.budget = 0;
