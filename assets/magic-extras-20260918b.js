  ...
  async function askTravelAI(trip, history) {
    ...
    for (const ep of endpoints) {
      try {
        ...
        if (content && content.length > 8) return content;
      } catch (err) {
        lastErr = err;
      }
    }
    ...
    try {
      ...
      if (aiPlaces.length) {
        ...
        if (!reply) {
          reply = places.length
            ...
            : `Al momento l’AI non risponde: ti lascio i link alle guide affidabili qui sotto.`;
        }
      }

      if (!reply) {
        reply = aiText
          ? formatChatDisplay(aiText)
          : "Dimmi pure cosa ti serve: ristoranti, musei, natura, consigli pratici o un’idea di giornata.";
      }

      thinking.textContent = formatChatDisplay(reply);
      if (!aiText) chatHistory.push({ role: "assistant", content: reply });
      ...
      status.textContent = aiText
        ? aiPlaces.length
          ...
          : "Risposta AI"
        : "Modalità guide (AI temporaneamente non raggiungibile)";
    } catch (err) {
