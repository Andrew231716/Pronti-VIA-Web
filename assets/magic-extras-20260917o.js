 ...
 * - Meteo (Open-Meteo), cambio valute (Frankfurter), link utili, copia riepilogo
 */
(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .pv-useful{margin:16px 0 18px;padding:0;border:0;background:transparent}
      .pv-useful > .pv-useful-head{margin:0 0 14px}
      .pv-useful > .pv-useful-head .eyebrow{display:block;font-size:.72rem;letter-spacing:.08em;text-transform:uppercase;color:#6a7f85;font-weight:700;margin:0 0 4px}
      .pv-useful > .pv-useful-head h3{margin:0;font-size:1.15rem;color:#16353b;font-weight:700}
      .pv-useful .pv-block{margin:0 0 14px;padding:14px 16px;border:1px solid #d9e7e7;border-radius:16px;background:#fff}
      .pv-useful .pv-block:last-of-type{margin-bottom:0}
      .pv-useful .pv-block-title{margin:0 0 10px;font-size:.78rem;letter-spacing:.06em;text-transform:uppercase;color:#5b7076;font-weight:700}
      .pv-useful .pv-block-meta{margin:0 0 10px;font-size:.86rem;color:#5b7076}
      .pv-useful .pv-weather-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(112px,1fr));gap:8px}
      .pv-useful .pv-weather-card{padding:10px 12px;border-radius:14px;background:#f4f8f8;border:1px solid #e4eeee;min-height:88px;display:flex;flex-direction:column;gap:4px}
      .pv-useful .pv-weather-card strong{font-size:.82rem;color:#16353b}
      .pv-useful .pv-weather-card .temp{font-size:1.05rem;font-weight:700;color:#007d82;margin-top:auto}
      .pv-useful .pv-weather-card .cond{font-size:.78rem;color:#5b7076;line-height:1.35}
      .pv-useful .pv-fx-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:8px}
      .pv-useful .pv-fx-card{padding:12px;border-radius:14px;background:#f4f8f8;border:1px solid #e4eeee}
      .pv-useful .pv-fx-card.primary{background:#eef7f7;border-color:#c5dede}
      .pv-useful .pv-fx-card .code{display:block;font-size:.72rem;letter-spacing:.06em;text-transform:uppercase;color:#6a7f85;font-weight:700;margin-bottom:4px}
      .pv-useful .pv-fx-card .rate{font-size:1rem;font-weight:700;color:#16353b}
      .pv-useful .pv-fx-card .name{display:block;margin-top:2px;font-size:.75rem;color:#5b7076}
      .pv-useful .pv-action-list{display:grid;gap:0}
      .pv-useful .pv-action-list a,.pv-useful .pv-action-list button{appearance:none;display:flex;align-items:center;justify-content:space-between;gap:10px;width:100%;padding:11px 0;border:0;border-bottom:1px solid #eef3f3;background:transparent;color:#184047;font:600 0.92rem inherit;cursor:pointer;text-decoration:none;text-align:left}
      .pv-useful .pv-action-list a:last-child,.pv-useful .pv-action-list button:last-child{border-bottom:0;padding-bottom:0}
      .pv-useful .pv-action-list a:first-child,.pv-useful .pv-action-list button:first-child{padding-top:0}
      .pv-useful .pv-action-list .hint{font-size:.78rem;font-weight:600;color:#7a9096}
      .pv-useful .pv-status{min-height:1.1em;margin:10px 0 0;font-size:.82rem;color:#5b7076}
    `;
  ...
  async function renderExpenseChart(host) {
    try {
      ...
      if (!expenses.length) {
        ...
        tip.innerHTML = `<strong>Spese per categoria</strong><p style="margin:0;color:#5b7076;font-size:.88rem">Aggiungi una spesa per vedere il riparto gratuito per categoria.</p>`;
  ...
  function injectUsefulKit() {
    if (!tripAuth()) return;
    if (document.querySelector(".pv-useful")) return;
    const heading = [...document.querySelectorAll("h2")].find((el) =>
    ...
    const box = document.createElement("div");
    box.className = "pv-useful";
    box.innerHTML = `
      <div class="pv-useful-head">
        <span class="eyebrow">Destinazione</span>
        <h3>Info utili</h3>
      </div>
      <div class="pv-block" id="pv-block-weather">
        <div class="pv-block-title">Meteo</div>
        <div class="pv-block-meta" id="pv-weather-meta">Caricamento…</div>
        <div class="pv-weather-grid" id="pv-weather"></div>
      </div>
      <div class="pv-block" id="pv-block-fx">
        <div class="pv-block-title">Cambio</div>
        <div class="pv-block-meta" id="pv-fx-meta"></div>
        <div class="pv-fx-grid" id="pv-fx"></div>
      </div>
      <div class="pv-block" id="pv-block-actions">
        <div class="pv-block-title">Link e azioni</div>
        <div class="pv-action-list" id="pv-actions-list"></div>
      </div>
      <p class="pv-status" id="pv-useful-status"></p>
    `;
    const dayStrip = host.querySelector(".day-strip");
    if (dayStrip) host.insertBefore(box, dayStrip);
    else if (heading?.nextSibling) heading.parentElement.insertBefore(box, heading.nextSibling);
    else host.appendChild(box);

    void fillUsefulKit(box);
  }

  async function fillUsefulKit(box) {
    const weatherEl = box.querySelector("#pv-weather");
    const weatherMeta = box.querySelector("#pv-weather-meta");
    ...
    try {
      ...
      // FX — destination currency first, then a couple of traveler references
      try {
        const wanted = [];
        if (localFx && localFx.code !== "EUR") wanted.push(localFx.code);
        for (const code of ["USD", "GBP", "CHF"]) {
          if (!wanted.includes(code) && code !== localFx?.code) wanted.push(code);
        }
        const symbols = wanted.slice(0, 4).join(",");
        if (!symbols) {
          fxMeta.textContent = "Destinazione in euro: nessun cambio necessario.";
          fxEl.innerHTML = "";
        } else {
          const fx = await fetch(
            `https://api.frankfurter.dev/v1/latest?base=EUR&symbols=${encodeURIComponent(symbols)}`,
          ...
          fxMeta.textContent = localFx
            ? localFx.code === "EUR"
              ? "Destinazione in euro."
              : `Valuta locale: ${localFx.name} (${localFx.code})`
            : "Cambi di riferimento da 1 €";
          fxEl.innerHTML = wanted
            .filter((code) => rates[code] != null)
            .map((code, i) => {
              const rate = Number(rates[code]);
              const label = names[code] || code;
              const primary = localFx && code === localFx.code ? " primary" : i === 0 && localFx ? " primary" : "";
              const digits = rate >= 100 ? 0 : rate >= 10 ? 2 : 2;
              return `<div class="pv-fx-card${primary}"><span class="code">${escapeHtml(code)}</span><div class="rate">1 € = ${rate.toLocaleString("it-IT", { maximumFractionDigits: digits })}</div><span class="name">${escapeHtml(label)}</span></div>`;
            })
            .join("");
        }
      } catch {
      ...
      if (!daily?.time?.length) {
        weatherMeta.textContent = "Meteo non disponibile per queste date.";
        weatherEl.innerHTML = "";
        return;
      }
      ...
      weatherMeta.textContent =
        mode === "forecast"
          ? `Previsioni · ${trip.destination}`
          : `Media climatica · ${trip.destination}`;
      weatherEl.innerHTML = "";
      daily.time.forEach((day, i) => {
        const card = document.createElement("div");
        card.className = "pv-weather-card";
        if (mode === "forecast") {
          ...
          card.innerHTML = `
            <strong>${escapeHtml(formatDayIt(day))}</strong>
            <span class="cond">${escapeHtml(weatherLabel(code))}${rain != null ? ` · ${rain}%` : ""}</span>
      ...
      weatherMeta.textContent = err instanceof Error ? err.message : "Info non disponibili.";
    }
  }

  async function copyDaySummary(box) {
    const status = box.querySelector("#pv-useful-status");
    try {
      const { data } = await loadTrip();
      const trip = data.trip;
      const days = tripDays(trip);
      const active =
        [...document.querySelectorAll(".day-strip button, .day-strip [aria-pressed='true'], button")]
          .find((el) => el.getAttribute("aria-pressed") === "true" || /giorno\s+\d+/i.test(el.className + el.textContent))
          ?.textContent?.match(/(\d+)/)?.[1] || "1";
      const dayIndex = Math.max(0, Number(active) - 1);
      ...
      status.textContent = "Riepilogo giorno copiato.";
    } catch (err) {
      status.textContent = err instanceof Error ? err.message : "Copia non riuscita";
    }
  }
  ...
  async function copyTripSummary(box) {
    ...
    try {
      ...
      status.textContent = "Riepilogo viaggio copiato.";
    } catch (err) {
      status.textContent = err instanceof Error ? err.message : "Copia non riuscita";
    }
  }
